...
function embedDialog(props: EmbedProps): ReactElement {
  ...
  }${newLineMarkdown}* \`?embed=noColoredBar\` makes the colored bar at the top disappear.${newLineMarkdown}* \`?embed=noToolBar\` makes the toolbar disappear.${newLineMarkdown}* \`?embed=noPadding\` removes the extra padding at the top and bottom of the app.${newLineMarkdown}* \`?embed=noScroll\` disables scrolling.${newLineMarkdown}* \`?embed=noFooter\` removes "Made with Streamlit" footer.${newLineMarkdown}${newLineMarkdown}Embed params can be merged in the following way:${newLineMarkdown}\`?embed=noColoredBar&embed=noToolBar\`.${newLineMarkdown}Param \`?embed=true\` will always override other options, so do not use it when trying to customize embed params.${newLineMarkdown}`
